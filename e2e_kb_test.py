...
d1 = r.json()
check(f"upload intro.md 200, saved_to={d1.get('saved_to','')}",
...
d2 = r.json()
check(f"upload-text city.md 200, file_name={d2.get('file_name','')}",
...
idx = r.json()
t1 = time.time()
check(f"force 索引 2 files → files={idx.get('files')}, chunks={idx.get('chunks')}",
...
doc_r = r.json()
check(f"documents 列表 total={doc_r.get('total')}",
...
check(f"搜索「青云城百宝阁在哪」命中 {len(hits)} 条，第一名是城市概览",
      r.status_code == 200 and len(hits) >= 1 and (
...
jr = r.json()
check(f"jobs 列表里有 {len(jr.get('jobs',[]))} 条，有 done",
...
check(f"审计 log 至少有 index_start + retrieve 这 2 个 op (实际={sorted(set(ops))})",
...
if city_doc:
    ...
    dr = r.json()
    check(f"软删 city.md ok (soft_deleted={dr.get('soft_deleted')}, deleted_chunk_count={dr.get('deleted_chunk_ids_count')})",
          r.status_code == 200 and dr.get("ok") and (dr.get("soft_deleted") or 0) == 1 and (dr.get("deleted_chunk_ids_count") or 0) >= 1,
          f"status={r.status_code} text={r.text[:200]}")
    r = client.post(f"/api/kb/{WORLD_ID}/search", json={"query": "百宝阁丹药 天机阁 御剑堂", "top_k": 10, "min_score": 0.0})
    hits = (r.json()).get("hits") or []
    remaining_city_hits = [h for h in hits if "百宝阁" in (h.get("text") or "") or "天机阁" in (h.get("text") or "")]
    check(f"软删后搜索「百宝阁丹药 天机阁」应 0 条命中 (实际={len(remaining_city_hits)})",
