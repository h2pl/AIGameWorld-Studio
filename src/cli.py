# CLI 入口 / CLI Entry
# world-template → Generator → world-pack（实例 YAML 合集）
# world-template (templates/) × CLI params → generate world-pack
...
def _main() -> int:
    ...
    gen = sub.add_parser(
        "generate",
        help="生成 world-pack / Generate world-pack from world-templates",
        epilog="示例: aw-studio generate --name 遗忘国度 --pc 2 --actor 3",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    gen.add_argument("--name", type=str, default="my_world", help="world-pack 名称 / World pack name")
...
def _generate(args) -> int:
    """从 world-template 生成 world-pack."""
