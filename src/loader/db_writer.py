...
def clear_existing(conn: sqlite3.Connection):
    for t in ["story_hooks", "story_arcs", "actors", "player_characters",
...
def _write_scenes(conn, scenes, pack_name, id_map):
    ...
    for s in scenes:
        ...
        conn.execute(stmt, (
            db_id, s.get("name", ""), s.get("type", ""), s.get("description", ""),
...
def _write_items(conn, items, pack_name, id_map):
    ...
    for i in items:
        ...
        conn.execute(stmt, (
            db_id, i.get("name", ""), i.get("item_type", ""), i.get("rarity", "common"),
            i.get("weight", 0), i.get("value", 0), i.get("description", ""),
            _json_default(i.get("data"), "{}"), pack_name,
        ))
        n += 1
    return n, id_map
...
def _write_scene_objects(conn, objects, id_map):
    ...
    for o in objects:
        ...
        conn.execute(stmt, (
            db_id, o.get("name", ""), o.get("object_type", ""), scene_db_id,
            0, 0, 1, _json(o.get("interact_data")),
        ))
        n += 1
    return n
...
def _write_pcs(conn, pcs, id_map):
    ...
    for pc in pcs:
        ...
        conn.execute(stmt, (
            db_id, pc.get("name", ""), pc.get("role", ""), pc.get("race", ""),
            "active", pc.get("scene_id", "") or scene_db_id, 0, 0,
            _json_default(pc.get("attributes"), "{}"),
            _json(pc.get("combat")),
            _json_default(pc.get("character_arc"), "{}"),
            arc.get("goal", ""), "[]", pc.get("personality", ""),
...
def _write_actors(conn, actors, id_map):
    ...
    for a in actors:
        ...
        conn.execute(stmt, (
            db_id, a.get("name", ""), a.get("role", ""), a.get("race", ""),
            "active", scene_db_id, 0, 0,
            _json_default(a.get("attributes"), "{}"),
            _json(a.get("combat")), a.get("personality", ""),
...
def _write_story_arcs(conn, arcs):
    ...
    for arc in arcs:
        conn.execute(stmt, (
            _new_id("arc"), arc.get("type", "main"), arc.get("title", ""),
            arc.get("stage", "hook"), _json_default(arc.get("main_cast"), "[]"),
            "[]", "[]", _json_default(arc.get("branching_points"), "[]"), "setup",
        ))
...
def _write_story_hooks(conn, hooks):
    ...
    for h in hooks:
        conn.execute(stmt, (
            _new_id("hook"), 0, h.get("description", ""),
            h.get("intended_payoff", ""), urgency_map.get(h.get("urgency", "medium"), 5), "planted",
        ))
